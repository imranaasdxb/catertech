...
function Cell({ label, value, mono }: { label: string; value: React.ReactNode; mono?: boolean }) {
  return (
    <div className="min-w-0 rounded-lg border border-black/[0.06] bg-white px-2.5 py-2 sm:px-3 sm:py-2.5">
      <p className="text-[10px] font-semibold uppercase tracking-wider text-admin-ink/40">{label}</p>
      <p className={`mt-0.5 truncate text-xs font-medium text-admin-ink sm:text-[13px] ${mono ? "font-mono" : ""}`}>
        {value}
      </p>
    </div>
  );
...
export default function AdminProductViewPanel({ product }: { product: ProductRow }) {
  ...
  return (
    <div className="grid gap-3 sm:gap-3.5 lg:grid-cols-12 lg:gap-4">
      ...
      <div className="overflow-hidden rounded-xl border border-black/[0.07] bg-white lg:col-span-12">
        <div className="grid gap-3 p-3 sm:grid-cols-[64px_minmax(0,1fr)] sm:items-center sm:gap-4 sm:p-4 lg:grid-cols-[72px_minmax(0,1fr)_auto] xl:grid-cols-[80px_minmax(0,1fr)_minmax(220px,280px)]">
          <div className="mx-auto h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-admin-bg ring-1 ring-black/[0.06] sm:mx-0 sm:h-[72px] sm:w-[72px]">
            {images[0] ? (
          ...
          <div className="min-w-0 text-center sm:text-left">
            ...
            <p className="mt-0.5 truncate font-mono text-[11px] text-admin-ink/45 sm:text-xs">/{product.slug}</p>
            <div className="mt-2 flex flex-wrap items-center justify-center gap-1.5 sm:justify-start">
              ...
              {subCategoryName ? (
                <span className="rounded-md bg-admin-bg px-2 py-0.5 text-[10px] font-medium text-admin-ink/60 ring-1 ring-black/[0.06] sm:text-[11px]">
                  {subCategoryName}
                </span>
              ) : null}
            </div>
          ...
          <div className="flex flex-col gap-2 border-t border-black/[0.06] pt-3 sm:col-span-2 sm:border-t-0 sm:pt-0 lg:col-span-1 lg:border-l lg:border-t-0 lg:pl-4 lg:pt-0">
          ...
          <div className="flex gap-2 overflow-hidden">
            {images.slice(0, 5).map((src, idx) => (
              <a
                ...
                className="group relative h-14 w-14 shrink-0 overflow-hidden rounded-lg ring-1 ring-black/[0.08] sm:h-16 sm:w-16"
            ...
            {images.length > 5 ? (
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-admin-bg text-xs font-bold text-admin-ink/50 ring-1 ring-black/[0.06] sm:h-16 sm:w-16">
                +{images.length - 5}
              </div>
            ) : null}
          </div>
      ...
      <div className="rounded-xl border border-black/[0.07] bg-white p-3 sm:p-3.5 lg:col-span-7 xl:col-span-8">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4">
          <Cell label="Product ID" value={<span className="font-mono text-[11px]">{product.id.slice(0, 8)}…</span>} />
      ...
      {/* Specifications */}
      <div className="rounded-xl border border-black/[0.07] bg-white p-3 sm:p-3.5 lg:col-span-7 xl:col-span-7">
        <p className="mb-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-admin-ink/55">
          <Package className="h-3.5 w-3.5 text-admin-accent" aria-hidden />
          Specifications
        </p>
        {templateLoading ? (
          <div className="flex items-center gap-2 py-6 text-xs text-admin-ink/45">
            <Loader2 className="h-4 w-4 animate-spin text-admin-accent" aria-hidden />
            Loading specs…
          </div>
        ) : templateErr ? (
          <p className="py-4 text-xs text-red-600">{templateErr}</p>
        ) : specRows.length ? (
      ...
      {/* SEO & discoverability */}
      <div className="rounded-xl border border-black/[0.07] bg-white p-3 sm:p-3.5 lg:col-span-5 xl:col-span-5">
        <p className="mb-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-admin-ink/55">
          ...
          <Cell
            ...
            value={
              product.seoDescription?.trim() ? (
                <span className="line-clamp-2 whitespace-normal">{product.seoDescription.trim()}</span>
              ) : (
                "—"
              )
            }
          />
          <div className="rounded-lg border border-black/[0.06] bg-white px-2.5 py-2 sm:px-3 sm:py-2.5">
            ...
            {keywords.length ? (
              <div className="mt-1.5 flex flex-wrap gap-1">
                {keywords.slice(0, 8).map((kw) => (
                  <span
                    key={kw}
                    className="inline-flex items-center gap-0.5 rounded-md bg-admin-bg px-1.5 py-0.5 text-[10px] font-medium text-admin-ink/65 ring-1 ring-black/[0.05]"
                  >
                ...
                {keywords.length > 8 ? (
                  <span className="text-[10px] font-semibold text-admin-ink/40">+{keywords.length - 8} more</span>
                ) : null}
              </div>
            ) : (
      ...
      <div className="rounded-xl border border-black/[0.07] bg-white p-3 sm:p-3.5 lg:col-span-12">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-admin-ink/55">
            <CalendarClock className="h-3.5 w-3.5 text-admin-accent" aria-hidden />
            Description
          </p>
          {product.published ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-admin-accent ring-1 ring-admin-accent/25 transition hover:bg-admin-accent/10 sm:text-[11px]"
            >
              View on shop
        ...
        {descriptionText ? (
          <p className="line-clamp-3 text-xs leading-relaxed text-admin-ink/75 sm:line-clamp-2 sm:text-[13px] lg:line-clamp-2">
            {descriptionText}
          </p>
        ) : (
